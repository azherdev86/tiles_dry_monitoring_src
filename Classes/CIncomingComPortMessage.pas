...
const
  MAX_BYTES_COUNT = 25; //Максимальное количество байт в сообщении
  TIMEOUT_END_PACKET = 100;

type
  TypeIncomingMessageState = (imsNone,
                              imsWaitEndPacket, //часть данных получена, ожидается продолжение
...
type
  ...
  private
    ...
    FMessageLength : Byte; //Длина входящего сообщения в байтах (включая CRC, но без DeviceId и CommandId

    FRecievedPartTime : TDateTime; //Время получения последней пачки сообщений
    FRecievedTime     : TDateTime; //Время получения сообщения (считается момент, когда пришла первая пачка)
...
begin
  ...
  for i := 0 to count - 1 do
    begin
      ...
      if FIncomingByteIndex >= FMaxBytesCount    //На случай переполнения буфера
        then
          begin
